...
@MainActor
package final class SionGlobalLibrary {
  package static let shared = SionGlobalLibrary()
  package static let didChangeNotification = Notification.Name("SionKitGlobalLibraryDidChange")

  private let directoryURL: URL
  private var loadedIndex: GlobalLibraryIndex?
  private var loadFailure: Error?

  package init(directoryURL: URL = SionGlobalLibrary.defaultDirectoryURL()) {
    self.directoryURL = directoryURL
  }

  package var entries: [SceneLibraryEntry] {
  ...
  package func entry(id: String) -> SceneLibraryEntry? {
  ...
  package func payload(id: String) throws -> Data {
  ...
  @discardableResult
  package func add(payload: Data, name: String) throws -> SceneLibraryEntry {
  ...
  package func rename(id: String, to name: String) throws {
    var index = try writableIndex()
    guard let position = index.rows.firstIndex(where: { $0.id == id }) else {
      throw SceneLibraryError.itemNotFound(id)
    }
  ...
  package nonisolated static func defaultDirectoryURL() -> URL {
...
package struct GlobalLibraryIndex: Equatable {
  package struct Row: Equatable {
    package let id: String
    package var name: String
    package let payloadFileName: String
  }

  package static let formatIdentifier = "sion-library-index"
  ...
  package func dataRepresentation() throws -> Data {
  ...
  package static func isSafePayloadFileName(_ name: String) -> Bool {
...
private enum GlobalLibraryStorage {
  package static let fallbackDirectoryName = "Sion"
  package static let directoryName = "Library"
  package static let payloadDirectoryName = "payloads"
  package static let indexFileName = "index.json"
  package static let legacyFileName = "Library.json"
}

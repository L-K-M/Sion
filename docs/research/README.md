...
| `mermaid-lab/` | The runnable scripts + captured outputs behind `mermaid-ground-truth.md` (`npm install` inside the dir, then `node 03-flowchart-db-jsdom.mjs` etc.). `package-lock.json` records the exact verified dependency tree. Scripts `20-…` through `26-…` (+ `out-20..26.txt`) were added during the adversarial plan review to verify the **export emit grammar**: which arrow bodies parse (`===`, `-.-o`, `o==o`, minlen extensions…), which are parse errors (`A -- B`, `-.-->`…), label `&`/`<br>` round-trip behavior, the node-id keyword blocklist, and the `flowchart-elk` diagram type — PLAN.md §9.2/§9.4 encode their results. |

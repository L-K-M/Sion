...
package enum SionCanvasDefaults {
  package static let minimumInfiniteSize = SionSize(width: 4_000, height: 3_000)
}
...
extension ElementContent {
  ...
  package var supportsShadow: Bool {
    switch self {
...
package enum SionShadowDefaults {
  package static let style = ShadowStyle(
    color: SionColor(red: 0, green: 0, blue: 0, alpha: 0.35),
    offset: SionVector(dx: 0, dy: 2),
    blurRadius: 6
  )
  package static let minimumBlurRadius = 0.0
  package static let maximumBlurRadius = 40.0
}

/// Shared placement defaults keep click and drag creation consistent.
package enum SionCreationDefaults {
  package static let rectangleSize = SionSize(width: 160, height: 96)
  package static let circleSize = SionSize(width: 120, height: 120)
  package static let textSize = SionSize(width: 220, height: 56)

  package static func shapeSize(for kind: ShapeKind) -> SionSize {
    switch kind {
    case .ellipse:
      circleSize
...
package final class SionEditorController: NSObject {
  package enum DocumentChange {
    case done
    case undone
    case redone
  }

  package enum SelectionMode {
    case replace
    case extend
  }

  package enum SelectionTraversal {
    case forward
    case backward
  }

  package enum AnchorEditingState: Equatable {
    case inactive
    case editing(ElementID)

    package var elementID: ElementID? {
  ...
  package enum AnchorEditResult: Equatable {
    case changed
    case outsideElement
    case unavailable
  }

  package enum ConnectorInsertionError: LocalizedError, Equatable {
  ...
  package enum Tool: Int, CaseIterable {
    ...
    package var shapeKind: ShapeKind? {
      switch self {
      case .rectangle:
  ...
  package enum ToolPersistence: Equatable {
    case oneShot
    case sticky
  ...
  #else
    // Linux decodes renditions in the toolkit layer; the controller keeps the
    // same content-addressed cache and eviction policy over whatever it hands back.
  ...
  package var document: SionDocument { editor.document }
  package var gridVisibility: GridVisibility { editor.document.scene.canvas.grid.visibility }

  package var selectedElements: [SceneElement] {
    editor.document.scene.elements.filter { selection.contains($0.id) }
  }

  package var selectedElement: SceneElement? {
    guard selection.count == 1, let id = selection.first else { return nil }

    return editor.document.scene.element(withID: id)
  }

  package var canMoveSelection: Bool {
  ...
  package var historyRevisions: [HistoryRevision] { history.revisions }

  package func selectionPayloadData() throws -> Data {
  ...
  @discardableResult
  package func insertSelectionPayload(
    _ data: Data,
    at point: SionPoint,
  ...
  package var documentLibrary: SceneLibrary {
    (try? SceneLibrary(portableValue: documentLibraryStorage)) ?? SceneLibrary()
  }

  @discardableResult
  package func addSelectionToDocumentLibrary(named name: String) throws -> SceneLibraryItem {
  ...
  package func removeDocumentLibraryItem(id: String) throws {
    var library = try writableDocumentLibrary()
    try library.remove(id: id)
    try storeDocumentLibrary(library, undoName: "Remove from Library")
  }

  package func renameDocumentLibraryItem(id: String, to name: String) throws {
    var library = try writableDocumentLibrary()
  ...
  /// that did not.
  package var documentLibraryStorage: PortableValue? {
  ...
  package var selectionLibraryName: String {
    let elements = selectedElements
  ...
  @discardableResult
  package func duplicateSelection() throws -> [ElementID] {
  ...
  package func alignSelection(_ edge: SceneAlignmentEdge) throws {
  ...
  package func distributeSelection(_ axis: SceneDistributionAxis) throws {
    let targets = arrangeableSelection()
  ...
  package func moveSelectionInZOrder(_ movement: ZOrderMovement) throws {
  ...
  package func setSelectionLockState(_ lockState: ElementLockState) throws {
  ...
  package func hideSelection() throws {
  ...
  package func revealHiddenElements() throws {
  ...
  package func toggleGridVisibility() throws {
  ...
  package var canDuplicateSelection: Bool {
    !selection.isEmpty
  }

  package var canCopySelection: Bool {
  ...
  package var canDeleteSelection: Bool {
  ...
  package func canMoveSelectionInZOrder(_ movement: ZOrderMovement) -> Bool {
    zOrderPlan(for: movement) != nil
  }

  package func canSetSelectionLockState(_ lockState: ElementLockState) -> Bool {
    !lockStateTargets(for: lockState).isEmpty
  }

  package var canHideSelection: Bool {
    !hideTargets.isEmpty
  }

  package var canRevealHiddenElements: Bool {
  ...
  package var arrangeableSelectionCount: Int {
  ...
  package func packageForArchiving() -> SionPackage {
    SionPackage(
  ...
  package func commitArchivedHistory(_ committedHistory: DocumentHistory) {
    history = committedHistory
    notifyObservers()
  }

  /// The archive's recovery preview, rendered by the canvas on save.
  package var hasPreviewPNG: Bool {
    previewPNG != nil
  }

  package func setPreviewPNG(_ data: Data) {
    previewPNG = data
  }

  package func load(_ package: SionPackage) throws {
    editor = try SceneEditor(document: package.document)
    assets = package.assets
    history = package.history
    previewPNG = package.previewPNG
  ...
  package func asset(for id: AssetID) -> SionAsset? {
    assets[id]
  }
    ...
    package func image(for content: ImageContent) -> NSImage? {
  ...
  #else
    /// Decodes display renditions once through the toolkit's decoder; IDs are
    /// content addressed, so entries stay valid until the asset is dropped.
    /// The decoder reports the pixel bytes it retains so the cache is charged
    /// the way the AppKit build charges it.
    package func image<Image: AnyObject>(
      for content: ImageContent,
      decode: (Data) -> (image: Image, decodedByteCount: Int)?
    ) -> Image? {
      let key = NSString(string: content.displayAssetID.rawValue)
      if let cached = imageCache.object(forKey: key) as? Image {
        return cached
      }

      guard let asset = asset(for: content.displayAssetID),
        let decoded = decode(asset.data)
      else {
        return nil
      }
      ...
      imageCache.setObject(
        decoded.image,
        forKey: key,
        cost: asset.data.count + decoded.decodedByteCount
      )
      return decoded.image
    }
  #endif

  package func connectorRoute(for element: SceneElement) -> ConnectorRoute? {
    renderContext.connectorRoute(for: element)
  }

  package func elementsForRendering(intersecting bounds: SionRect) -> [SceneElement] {
  ...
  package func editingCanvasBounds(minimumInfiniteSize: SionSize) -> SionRect {
  ...
  package func contentBounds() -> SionRect {
  ...
  @discardableResult
  package func observeChanges(_ observer: @escaping () -> Void) -> UUID {
    let id = UUID()
  ...
  package func setTool(_ newTool: Tool, persistence: ToolPersistence = .sticky) {
  ...
  @discardableResult
  package func toolDidComplete(_ completedTool: Tool) -> Bool {
  ...
  package func beginAnchorEditing(on id: ElementID) {
  ...
  package func select(_ id: ElementID?, mode: SelectionMode = .replace) {
  ...
  package func select(_ ids: Set<ElementID>, mode: SelectionMode = .replace) {
    let previous = selection
  ...
  package func elementIDsIntersecting(_ rect: SionRect) -> Set<ElementID> {
  ...
  package func selectAdjacent(_ traversal: SelectionTraversal) {
  ...
  package func nudgeSelection(by offset: SionVector) throws {
  ...
  @discardableResult
  package func insertShape(at point: SionPoint) throws -> ElementID {
    try insertShape(
      at: point,
  ...
  @discardableResult
  package func insertShape(at point: SionPoint, kind: ShapeKind) throws -> ElementID {
  ...
  @discardableResult
  package func insertShape(in frame: SionRect, kind: ShapeKind) throws -> ElementID {
  ...
  @discardableResult
  package func insertShape(centeredAt point: SionPoint, kind: ShapeKind) throws -> ElementID {
  ...
  @discardableResult
  package func insertText(_ text: String, at point: SionPoint) throws -> ElementID {
    let frame = SionRect(
      origin: point,
  ...
  package func beginTextEdit(on id: ElementID) throws {
  ...
  package func updateTextEdit(_ text: String, on id: ElementID) throws {
  ...
  package func endTextEdit() throws {
  ...
  package func checkpointTextEdit(on id: ElementID) throws {
  ...
  package func setRoutingStyle(_ style: ConnectorRoutingStyle, on id: ElementID) throws {
    try perform(
  ...
  package func setMagnetConfiguration(
    _ configuration: MagnetConfiguration, on id: ElementID
  ) throws {
    try perform(
      name: "Change Magnets",
  ...
  package func setLockState(_ lockState: ElementLockState, on id: ElementID) throws {
  ...
  package func setFillColor(_ color: SionColor, on id: ElementID) throws {
  ...
  package func setStrokeColor(_ color: SionColor, on id: ElementID) throws {
    guard var element = editor.document.scene.element(withID: id) else { return }
  ...
  package func setStrokeWidth(_ width: Double, on id: ElementID) throws {
    guard width >= 0,
  ...
  package func setShadow(_ shadow: ShadowStyle?, on id: ElementID) throws {
  ...
  package func setShadowEnabled(_ isEnabled: Bool, on id: ElementID) throws {
  ...
  package func setShadowColor(_ color: SionColor, on id: ElementID) throws {
    guard let element = editor.document.scene.element(withID: id) else { return }
  ...
  package func setShadowBlurRadius(_ radius: Double, on id: ElementID) throws {
  ...
  package func restoreRevision(identifier: String) throws {
  ...
  package func deleteSelection() throws {
    guard !selection.isEmpty else { return }
  ...
  package func moveSelection(by offset: SionVector) throws {
  ...
  package func beginResize() throws {
    guard selectedElement != nil else { return }

    try beginTransform(.resize)
  }

  package func resize(_ id: ElementID, to frame: SionRect) throws {
  ...
  package func endResize() throws {
    try endTransform(.resize)
  }

  package func beginRotation() throws {
    guard selectedElement != nil else { return }

    try beginTransform(.rotate)
  }

  package func rotate(_ id: ElementID, to radians: Double) throws {
  ...
  package func endRotation() throws {
    try endTransform(.rotate)
  }

  package func beginCornerRadiusChange() throws {
    guard selectedElement != nil else { return }

    try beginTransform(.cornerRadius)
  }

  package func setCornerRadius(_ radius: Double, on id: ElementID) throws {
    _ = try editor.updateGesture(
      with: .setShapeKind(
        elementID: id,
  ...
  package func endCornerRadiusChange() throws {
  ...
  package var hasPendingEditorGesture: Bool {
    editor.hasPendingGesture
  }

  package func element(at point: SionPoint) -> SceneElement? {
  ...
  package func connectableElement(at point: SionPoint) -> SceneElement? {
...
private enum EditorDefaults {
  package static let imageSize = SionSize(width: 240, height: 180)
  package static let maximumImageDimension = 480.0
  package static let strokeWidth = 1.5
  package static let connectorHitTolerance = 6.0
  package static let connectorMagnetSnapTolerance = 10.0
  package static let elementHitSlop = 2.0
  package static let customAnchorIDPrefix = "custom-"
  package static let imageCacheLimit = 256
  package static let imageCacheTotalCostLimit = 128 * 1024 * 1024
  package static let duplicateStepMinimum = 8.0
  package static let duplicateStepMaximum = 64.0
  package static let duplicateMoveTolerance = 0.5
}

private struct AnchorPlacement {
  package let point: SionPoint
  package let outwardDirection: SionVector
}
...
private enum RectangleAnchorEdge: CaseIterable {
  ...
  package static func nearest(to point: SionPoint, in frame: SionRect) -> RectangleAnchorEdge {
    allCases.min {
      $0.distance(to: point, in: frame) < $1.distance(to: point, in: frame)
    } ?? .north
  }

  package var outwardDirection: SionVector {
    switch self {
    case .north: .north
    case .east: .east
    case .south: .south
  ...
  package func distance(to point: SionPoint, in frame: SionRect) -> Double {
    switch self {
  ...
  package func project(_ point: SionPoint, onto frame: SionRect) -> SionPoint {
    switch self {
    case .north:
...
private enum EditorActionName {
  package static let editText = "Edit Text"
  package static let renameElement = "Rename Element"
  package static let renameElements = "Rename Elements"
  package static let hideGrid = "Hide Grid"
  package static let showGrid = "Show Grid"
}

private struct PendingTextEdit {
  package let elementID: ElementID
  package let baselineText: String
  package var didMarkDocumentChanged = false
}

private struct DuplicateState {
  package let ids: Set<ElementID>
  package let delta: SionVector
  package var manualTranslation = SionVector.zero
}

private struct ZOrderPlan {
  package let orderedIDs: [ElementID]
  package let destinationIndex: Int
}

package enum ZOrderMovement {
  case front
  case forward
  case backward
  case back
